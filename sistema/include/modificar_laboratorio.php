    ...
    <div style="width:72%; padding:30px 1%; min-height:200px; margin-left:1px; float:left; background:rgba(51,51,51,0.1); border-left:solid 2px #0071BC;">
    	<?php
			...
			if($bandera==1){
				...
				mysqli_query($conexion, "update imn_laboratorios set nombre='$nombre', nombre_encargado='$nombre_encargado', telefono='$telefono', correo='$correo', contrasena='$contrasena', fecha='$fecha', activo='$activo' where id_laboratorio=$laboratorio");
				...
				$sql = "select nombre, nombre_encargado, telefono, correo, contrasena, activo from imn_laboratorios where id_laboratorio=$laboratorio";
				...
				@$activo=$row['activo'];
		?>
    	<form method="post" action="modificar_laboratorio.php?laboratorio=<?php echo $laboratorio; ?>" enctype="multipart/form-data">
            ...
            <input type="text" name="nombre" id="nombre" placeholder="* Nombre ó Razón Social" class="texto" required="required" style="width:88%" value="<?php echo $nombre; ?>"/>
            <div class="clearer" style="height:5px"></div>
            <input type="text" name="nombre_encargado" id="nombre_encargado" placeholder="* Nombre del encargado" class="texto" style="width:88%" required="required" value="<?php echo $nombre_encargado; ?>"/>
            <div class="clearer" style="height:5px"></div>
