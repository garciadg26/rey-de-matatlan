								...
								if(mysqli_num_rows($datosp)>0){
									while ($datos2p=mysqli_fetch_array($datosp)){
										...
										echo '<div style="width:100%; height:auto; display:table; font-size:12px; background:rgba(255,255,255,0.7);">';
											echo '<div style="width:10%; padding:0; float:left; text-align:left;"><a href="include/imagen_nutriologo.php?nutriologo='.$datos2p['id_nutriologo'].'" class="fancybox fancybox.iframe" style="text-decoration:none; color:#FFF;"><img src="imagenes/'.$datos2p['imagen'].'" height="35px"></a></div>';
											echo '<div style="width:18%; padding:10px 1%; float:left; text-align:left;">'.$datos2p['nombre'].' '.$datos2p['a_paterno'].' '.$datos2p['a_materno'].'</div>';
											echo '<div style="width:13%; padding:10px 1%; float:left; text-align:left;">'.$especialidad.'</div>';
											...
											echo '<div style="width:10%; padding:10px 0; float:left; text-align:center; background:#FF9326;"><a href="include/direccion_nutriologo.php?nutriologo='.$datos2p['id_nutriologo'].'" class="fancybox fancybox.iframe" style="text-decoration:none; color:#333;">DIRECCIÓN</a></div>';
											echo '<div style="width:10%; padding:10px 0; float:left; text-align:center; background:rgba(0,50,105,0.8);"><a href="include/modificar_nutriologo.php?nutriologo='.$datos2p['id_nutriologo'].'" class="fancybox fancybox.iframe" style="text-decoration:none; color:#FFF;">MODIFICAR</a></div>';
											echo '<div style="width:10%; padding:10px 0; float:left; text-align:center; background:rgba(213,23,0,0.9);"><a href="include/eliminar_nutriologo.php?nutriologo='.$datos2p['id_nutriologo'].'" class="fancybox fancybox.iframe" style="text-decoration:none; color:#FFF;">ELIMINAR</a></div>';
