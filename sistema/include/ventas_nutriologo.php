...
<div style="width:100%; padding:10px 0; display:table;">
    ...
    <div style="width:72%; padding:30px 1%; min-height:200px; margin-left:1px; float:left; background:rgba(51,51,51,0.1); border-left:solid 2px #0071BC;">
    	<?php
			...
			// LISTADO
			echo '<div id="titulo_3">HISTORIAL DE VENTAS DE '.$nombre.' '.$paterno.' '.$materno.'</div>';
			$datosp=mysqli_query($conexion, "select cliente, total, fecha from imn_pedidos where nutriologo=$nutriologo order by fecha desc");
				if(mysqli_num_rows($datosp)>0){
					while ($datos2p=mysqli_fetch_array($datosp)){
						$sql2 = "select nombre, a_paterno, a_materno from imn_clientes where id_cliente=".$datos2p['cliente']."";
						$result2 = mysqli_query($conexion, $sql2);
						@$row2=mysqli_fetch_array($result2,MYSQLI_ASSOC);
						@$nombre_c=$row2['nombre'];
						@$paterno_c=$row2['a_paterno'];
						@$materno_c=$row2['a_materno'];
						
						// COMISIÓN
						$comision_2=$comision/100;
						$comision_3=$datos2p['total']*$comision_2;
			
						echo '<div id="listado">';
							echo '<div id="columna_1">'.$nombre_c.' '.$paterno_c.' '.$materno_c.'</div>';
							echo '<div id="columna_2">'.f_fecha($datos2p['fecha']).'</div>';
							echo '<div id="columna_3">$'.number_format($datos2p['total'],2).' MN</div>';
							echo '<div id="columna_3" style="color:#F30;">$'.number_format($comision_3,2).' MN</div>';
						echo '</div>';
