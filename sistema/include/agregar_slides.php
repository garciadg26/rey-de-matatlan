    ...
    <div style="width:62%; padding:30px 1%; min-height:200px; margin-left:1px; float:left; background:rgba(51,51,51,0.1); border-left:solid 2px #996633;">
    	<?php
			...
			if($bandera==1){
				...
				mysqli_query($conexion, "insert into hb_slides(id_hotel, imagen, fecha) values($hotel, '$archivo', '$fecha')");
				echo '<div id="mensaje">Información registrada correctamente<br><br><a href="agregar_slides.php?hotel='.$hotel.'"><img src="../imagenes/icon_ok.png"></a></div>';
			} else {
		?>
    	<form method="post" action="agregar_slides.php?hotel=<?php echo $hotel; ?>" enctype="multipart/form-data">
        ...
        <?php
			...
			echo '<div id="titulo">LISTADO DE SLIDES</div>';
			$datosp=mysqli_query($conexion, "select id_slide, id_hotel, imagen from hb_slides where id_hotel=$hotel order by id_slide asc");
				if(mysqli_num_rows($datosp)>0){
					while ($datos2p=mysqli_fetch_array($datosp)){
						echo '<div id="listado_imagenes">';
							...
							echo '<div id="listado_imagenes_btn"><a href="borrar_slide.php?imagen='.$datos2p['id_slide'].'&hotel='.$hotel.'">BORRAR</a></div>';
						echo '</div>';
