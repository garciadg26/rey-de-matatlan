    ...
    <div style="width:72%; padding:30px 1%; min-height:200px; margin-left:1px; float:left; background:rgba(51,51,51,0.1); border-left:solid 2px #0071BC;">
    	<?php
			...
			if($bandera==1){
				...
				//$estado2=htmlentities($estado, ENT_QUOTES, "UTF-8");
				//$ciudad2=htmlentities($ciudad, ENT_QUOTES, "UTF-8");
				...
				mysqli_query($conexion, "update imn_nutriologos set calle='$calle', numero='$numero', colonia='$colonia', cp='$cp', ciudad='$ciudad', estado='$estado' where id_nutriologo=$nutriologo");
				...
				$sql = "select calle, numero, colonia, cp, ciudad, estado from imn_nutriologos where id_nutriologo=$nutriologo";
    	...
    	<form method="post" action="direccion_nutriologo.php?nutriologo=<?php echo $nutriologo; ?>" enctype="multipart/form-data">
