    	...
    	<?php
      	...
      	@$nombre=$_POST['nombre'];
				...
				if($bandera==1){
					@$tamano=$_POST['tamano'];
					@$precio=$_POST['precio'];
					mysqli_query($conexion,"insert into vin_detalle_tamano(id_vino, id_tamano, precio, fecha) values($vino, $tamano, '$precio', '$fecha')");
					echo '<div id="mensaje">Información agregada correctamente!<br><br><br>';
				...
				echo '<div id="forma">';
					echo '<form method="post" action="agregar_precio.php?vino='.$vino.'">';
						...
						echo '<select name="tamano" id="tamano" class="combo" required>';
							...
							//$datos=mysqli_query($conexion, "select * from vin_tamanos where tipo=$tipo ");
						...
						echo '<input type="text" name="precio" id="precio" class="texto" placeholder="* Ingresa el precio del vino" required>';
						echo '<div class="clearer" style="height:5px;"></div>';
						echo '<span>* Para ingresar un precio de $450.00 MN, solo ingresar 450</span>';
						echo '<div class="clearer" style="height:15px;"></div>';
						echo '<input type="submit" value="Registrar" class="enviar">';
					echo '</form>';
				echo '</div>';
				}
